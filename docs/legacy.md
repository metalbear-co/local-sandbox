...
Everything here documents the **legacy** task set (`Taskfile.legacy.yml` + `tasks/*.yml`).
All of it still works, prefixed with `legacy:` — e.g. `task legacy:operator:update`,
`task legacy:test:postgres`, `task legacy:menu`. Pass variables after `--`
(`task legacy:operator:load -- TAG=my-branch`). `task legacy` lists every task.

For the current day-to-day flows see the [main README](../README.md).
